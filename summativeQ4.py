...
# the goal is to add the concatnated values till you have jus 1 value .
...
def megadigit(n,k):
    ...
    while len(p) >= 2 :       
        # since super digit aims to get to the point where the sum of the concatnated values(P) is 1 value
        #  we use a while loop keep reducing the number of digits in P.
        sum = 0
        # loops through all the values of P and converts them to integer  as they are added up.
